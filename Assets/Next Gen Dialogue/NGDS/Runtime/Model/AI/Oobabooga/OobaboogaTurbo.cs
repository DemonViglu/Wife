...
namespace Kurisu.NGDS.AI
{
    public class OobaboogaTurbo : ILLMDriver
    {
        ...
        public async Task<ILLMData> ProcessLLM(ILLMInput input, CancellationTokenSource ct)
        {
            SetStopCharacter(input.OtherCharacters);
            return await SendMessageToOobaboogaAsync(chatGenerator.Generate(input));
        }
        private async Task<OobaboogaResponse> SendMessageToOobaboogaAsync(string message)
        {
