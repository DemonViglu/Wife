        ...
        private class EditorTranslateNode : EditorModuleNode
        {
            ...
            private async void TranslateAllContentsAsync()
            {
                ...
                var containerNodes = MapTreeView.View.nodes
                    ...
                    .ToArray();
                var tuples = new List<(ContainerNode containerNode, ModuleNode moduleNode)>();
                foreach (var node in containerNodes)
                {
                    if (node.TryGetModuleNode<ContentModule>(out ModuleNode moduleNode))
                    {
                        tuples.Add((node, moduleNode));
                    }
                }
                int tupleCount = tuples.Count;
                for (int i = 0; i < tupleCount; ++i)
                {
                    float slider = (float)i / tupleCount;
                    EditorUtility.DisplayProgressBar("Wait to translate content", "Waiting for a few seconds", slider);
                    ContainerNode containerNode = tuples[i].containerNode;
                    ModuleNode moduleNode = tuples[i].moduleNode;
            ...
            internal async void TranslateNodeAsync(IDialogueNode node)
            {
                ...
                {
                    float slider = (float)i / fieldCount;
                    EditorUtility.DisplayProgressBar("Wait to translate content", "Waiting for a few seconds", slider);
                    FieldInfo fieldInfo = fieldsToTranslate[i];
                    string input = null;
