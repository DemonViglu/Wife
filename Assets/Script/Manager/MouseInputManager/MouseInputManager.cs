...
namespace DemonViglu.MouseInput {
    public partial class MouseInputManager : MonoBehaviour {
        ...
        public bool isHold;
        public bool isMove;
        public RaycastHit2D mouseHitInfo;
        ...
        private void MouseStateUpdating() {
            ...
            //
            if (Input.mousePosition != prePosition) {
                isMove = true;    
            }
            else {
                isMove = false;
        ...
        #region 当鼠标点击时，更新状态，并调用事件
        private void MouseClick() {
            if (Input.GetMouseButtonDown(0)) {
                OnMouseMultiClickDown?.Invoke(clickCounts);
                if (clickCounts == 0) {
        ...
        private void GetMouseHitinfo() {
            Ray ray = Camera.main.ScreenPointToRay(Input.mousePosition);
            //Debug.DrawRay(ray.origin, ray.direction * 20f, Color.blue, duration: 1.0f);
            mouseHitInfo = Physics2D.Raycast(new Vector2(ray.origin.x, ray.origin.y), Vector2.zero);
        }
